...
/** 검증된 라이브/예정 영상 후보 (5분 캐시). 실존하지 않는 ID 는 제외된다. */
export async function getVerifiedVideos(): Promise<Verified[]> {
  const now = Date.now();
  ...
  inflight = (async () => {
    ...
    await Promise.all(
      raw.slice(0, 8).map(async (v) => {
        ...
        // 검증 못 하는 항목은 아예 제외한다 — 방송에 빈 화면을 띄우느니 안 보이는 게 낫다.
        if (!id) return;
...
/**
 * 자동 송출 후보 — **지금 라이브인 공적 소스만**.
 * 기업 IR/기타는 자동으로 올리지 않는다 (사람이 골라야 하는 판단이다).
 */
export function pickAutoVideo(list: Verified[]): Verified | null {
  return list.find((v) => v.live && (v.source === "fed" || v.source === "gov")) ?? null;
}
