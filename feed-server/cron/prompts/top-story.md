...
## 서로 다른 사건을 한 문장에 엮지 마라

**실측 사고 (2026-07-29):**
> "US intercepts Iranian missile attack as oil surges, chip stocks sink worldwide"

이란 → 유가는 맞다. 그런데 그날 반도체 급락의 원인은 이란이 **아니라** 중국 CXMT
상장에 따른 범용 메모리 공급 과잉·마진 우려였다(Micron·SanDisk·SK하이닉스 기사가
전부 CXMT 를 지목했다). 두 사건을 `as` 로 묶는 순간 화면은 **하나의 사건**이라고 말하게 된다.

- 원인이 다른 두 사건을 `as` · `amid` · `after` 로 잇지 마라. 그 접속사는 인과로 읽힌다.
- 그날 시장을 가장 크게 움직인 **하나**를 골라라. 지수·업종 낙폭이 가장 큰 쪽이 보통 그것이다.
- 두 사건이 정말 둘 다 중요하면 `;` 로 나누거나, 하나만 헤드라인에 쓰고 나머지는 `why` 에 적어라.
